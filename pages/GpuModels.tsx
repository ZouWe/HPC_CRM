    ...
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 duration-300">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5 col-span-2">
                  <label className="text-xs font-black text-slate-700 uppercase">供货状态</label>
                  <div className="grid grid-cols-3 gap-2">
                    {(['AVAILABLE', 'SHORTAGE', 'UNAVAILABLE'] as const).map(s => {
                      const info = getStatusInfo(s);
                      return (
                        <button key={s} type="button" onClick={() => setFormData({...formData, status: s})} className={`px-3 py-2 text-[10px] font-black rounded-xl border transition-all ${formData.status === s ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'}`}>
                          {info.label}
                        </button>
                      );
                    })}
